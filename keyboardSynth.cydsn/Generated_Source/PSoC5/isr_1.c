...
CY_ISR(isr_1_Interrupt)
{
    ...
    double fOut;
    extern int TONE;  
    static uint32 int_n = 0;         //initialize count to 0
    double dubn = (double) int_n;
    
    
    
    fOut=0.0;                                         //initialize output to DAC to be 0
    
    if(Tone_Switch_Read()==0){                      //Changes option of TONE given button push
        TONE=1-TONE;
        CyDelay(100);
        while(Tone_Switch_Read() ==0){
        }
        CyDelay(100);
    }
